...
# Upstream replaced the per-dataset fetchers below with a single prebuilt eval download pinned to a
# revision, and bumped that pin (635a833 -> 5c8f784) as part of the CRoMa change. Our fetchers are kept
# because they built the eval data already on disk; the pin is recorded so the provenance is not lost
# and we can re-pull against it if CRoMa needs eval metadata our build does not carry.
