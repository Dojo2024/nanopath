...
`model.probe_layers` selects which 1-indexed blocks the frozen-feature probes read, and `model.probe_reduce` sets how they combine (`concat` widens to `n * embed_dim`, `mean` averages back to `embed_dim`). The default `[4, 6, 8, 11]` concat scores +0.0107 over the final block alone; the gain needs early depths kept in separate subspaces, since `[9,10,11,12]` recovers only +0.0024, `[12,12,12,12]` (same width, no new information) +0.0006, and averaging the same four depths -0.0010. Training never calls the readout, so changing it only re-probes. `train.py` exports both values into the environment because the probe subprocess rebuilds the model from `model.type` alone and cannot be passed anything else.
