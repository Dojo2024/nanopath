# DINO/I-JEPA pretraining on TCGA tiles (single-GPU), currently initialized from DINOv2. Three loss terms:
# DINO CLS self-distillation (Sinkhorn-Knopp centred teacher targets),
# I-JEPA block-masked patch-feature regression, and a KDE uniformity term on the
...
# I-JEPA target mask: contiguous square blocks, so the predictor must infer missing tissue from the
# surrounding morphology instead of interpolating between scattered single patches.
def make_block_mask(batch, grid, device, n_blocks, block_scale):
    ...
    # V-JEPA 2.1 context loss: visible patches are supervised too, weighted 1/sqrt(distance
    # to the nearest masked patch), so continuity is enforced hardest at mask boundaries.
    coords = torch.cartesian_prod(torch.arange(grid, device=device), torch.arange(grid, device=device)).float()
    d = torch.where(masks.unsqueeze(1), torch.cdist(coords, coords).unsqueeze(0), torch.inf).amin(-1)
    w = d.clamp(min=1).rsqrt().mul(~masks)
    return masks, idx, weights, (~masks).flatten().nonzero().flatten(), (w / w.sum(-1, keepdim=True).clamp(min=1e-6))[~masks]
...
def main():
    ...
    def compute_losses(gf, lf, b, masks, mask_idx, mask_w, vis_idx, vis_w, t_temp, k_scale, ckpt=False):
        ...
        if dino_cfg["context_loss_weight"]:
            jepa_loss = jepa_loss + dino_cfg["context_loss_weight"] * F.smooth_l1_loss(pred[vis_idx], target[vis_idx], reduction="none").mean(-1).mul(vis_w).sum() / max(1, b * 2)
    ...
    def evaluate(eval_step, eval_teacher_temp, eval_kde_scale):
        ...
        for vb_idx, vbatch in enumerate(val_loader):
            ...
            with torch.no_grad(), autocast:
                ...
                masks, mask_idx, mask_w, vis_idx, vis_w = make_block_mask(b * train_cfg["global_views"], global_grid, device, int(dino_cfg["jepa_blocks"]), float(dino_cfg["jepa_block_scale"]))
                dino_l, jepa_l, kde_v = compute_losses(gf, lf, b, masks, mask_idx, mask_w, vis_idx, vis_w, eval_teacher_temp, eval_kde_scale)
    ...
    # Counts the EMA teacher forward + DINO head + JEPA predictor, not just the backbone, so the
    ...
    while examples_seen + batch_size <= max_train_samples and train_flops < max_train_flops:
        for batch in train_loader:
            ...
            # Schedules default to FLOP progress, but the run is SAMPLE-bound: it hits the 1M-tile cap
            # having spent only ~19% of the 1e18 FLOP budget, so a FLOP-keyed cosine traverses ~0.19 of
            # its arc and nothing anneals (lr decays 6.7%, wd reaches 0.054/0.2, kde 0.23/1.0).
            # "lr_only" re-keys the LR decay to sample progress; "all" also re-keys wd/teacher/kde/ema.
            frac = min(1.0, train_flops / max_train_flops)
            sfrac = min(1.0, examples_seen / max_train_samples)
            lr_frac = sfrac if dino_cfg["schedule_key"] in ("lr_only", "all") else frac
            reg_frac = sfrac if dino_cfg["schedule_key"] == "all" else frac
            ...
            masks, mask_idx, mask_w, vis_idx, vis_w = make_block_mask(batch_size * train_cfg["global_views"], global_grid, device, int(dino_cfg["jepa_blocks"]), float(dino_cfg["jepa_block_scale"]))
            ...
            with flop_ctx:
                with autocast:
                    ...
                    dino_loss_value, jepa_loss, kde = compute_losses(
                        gf, lf, batch_size, masks, mask_idx, mask_w, vis_idx, vis_w, teacher_temp, kde_scale,
