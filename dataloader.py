...
class TCGATileDataset(Dataset):
    ...
    def __init__(self, cfg, is_train=True):
        ...
        # Prognostic sidecar targets (cfg.prog): per-patient scalars that FINO does NOT already carry,
        # routed only to train.py's attention-pooled head and never to CLS. Chosen against this board's
        # measured family gaps rather than copied: mutcount/fga-style mutational burden feeds the mutation
        # column (surgen), and til/necrosis/stromal are the classical microenvironment prognostic factors
        # behind progression and survival. Values are residualized within `residualize_by` (cancer type)
        # so the target carries WITHIN-cancer variation; the cross-cancer part is already trivially
        # readable from morphology and would teach the backbone nothing new.
        self.prog = (cfg.get("prog") or {}).get("enabled")
        if self.prog:
            meta = json.loads((dataset_dir / "fino_meta.json").read_text())
            factors = list(cfg["prog"]["targets"])
            group_of = meta["discrete"][cfg["prog"]["residualize_by"]]
            columns = [meta["continuous"][f] for f in factors]
            pids = sorted({pid for column in columns for pid in column})
            values = np.array([[column.get(pid, np.nan) for column in columns] for pid in pids], dtype=np.float32)
            groups = np.array([group_of.get(pid, -1) for pid in pids])
            for g in np.unique(groups[groups >= 0]):
                rows = groups == g
                counts = (~np.isnan(values[rows])).sum(0)
                values[rows] -= np.where(counts > 0, np.nansum(values[rows], 0) / np.maximum(counts, 1), 0.0)
            self.prog_target = {pid: v for pid, v in zip(pids, values)}
            self.prog_missing = np.full(len(factors), np.nan, dtype=np.float32)
        ...
        # Idea A5: a stain-only sibling of global view 0 sharing its EXACT crop (geometry split out
        # of global_aug's original single-pipeline, left untouched above so every other config's
        # augmentation is byte-identical to before), so patch i here and patch i in the clean view
        # are the same tissue -- a direct patch-level target for stain invariance, unlike DINO's
        # CLS-only two-global agreement. Only built when train.py's stain_weight is nonzero.
        ...
        if self.stain_view:
            ...
            self.stain_color_strong = v2.Compose(
                [
                    HEDJitter(0.3),
                    v2.ColorJitter(0.6, 0.6, 0.6, 0.0),
                    v2.Normalize(mean=mean, std=std),
                ]
            )
    ...
    def __getitem__(self, idx):
        ...
        return {
            ...
            **fino_keys,
            **({"prog_target": torch.from_numpy(self.prog_target.get(patient_id, self.prog_missing).copy())} if self.prog else {}),
        }
